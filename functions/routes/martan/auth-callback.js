...
const { saveMartanAuth } = require('../../lib/auth/save-auth')
...
exports.get = async ({ admin, appSdk }, req, res) => {
  try {
    ...
    const { martan_store_id: martanStoreId, ecomplus_store_id: ecomplusStoreId } = parsedState

    if (!martanStoreId || !ecomplusStoreId) {
      throw new Error('Missing required store IDs in state')
    }

    const storeId = parseInt(ecomplusStoreId)
    const martanId = parseInt(martanStoreId)
